  ...
  *id =index;
  ...
  if (line[*counter] != '=') {
	  if ((index >= 1000) && (index <= 1003)) {
		  ...
		  value = (float)_setup.parameters_ext[index-1000] * ADC_REFERENCE_VOLTAGE_MV / ADC_MAX_OUTPUT_VALUE;
		  temperature = (int32_t)((value - TEMP_SENSOR_VOLTAGE_MV_AT_25) / TEMP_SENSOR_AVG_SLOPE_MV_PER_CELSIUS + 25);
		  report_parameter((unsigned int)index, temperature, TYPE_FLOAT);
	  }
	  else if (index == PARAM_PLC_BASE_INPUT) {
		  report_parameter((unsigned int)index, probe_get_state(), TYPE_UINT32);
	  }
	   else report_parameter((unsigned int)index , _setup.parameters[index], TYPE_UINT32);
  }
  else {
	  *counter = (*counter + 1);
	  ...
	  // write digital outputs
  	  if ((index >= PARAM_PLC_BASE_OUPUT) && (index < (PARAM_PLC_BASE_INPUT))) { // up to 128 outputs
  		output_set_state(index - PARAM_PLC_BASE, (unsigned int)value);
  	  }

  	  switch (index) {
  		  case PARAM_PENDANT_PHASE : pendant.phase = (unsigned int)value;
  		  	  	  	  	  	  	  	  break;
  		  case PARAM_PENDANT_REVOLUTION : pendant.pulse_per_rev = (unsigned int)value;
  		   	  	  	  	  	  	  	  break;
  		  case PARAM_PENDANT_SELECT : pendant.select = (unsigned int)value;
	  	  	  	  	                  break;
  		  case PARAM_PENDANT_FACTOR : pendant.resolution = (unsigned int)value;
  		   	  	  	  	  	  	  	  break;
  	  }
